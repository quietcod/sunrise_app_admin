...
class _AddTaskScreenState extends State<AddTaskScreen> {
  ...
  void initState() {
    super.initState();
    // Re-assert any preset rel_type / rel_id from the calling screen
    // (e.g. project -> tasks "Add" button) on the next frame, so that
    // even if upstream code or rebuilds clear them, they're restored
    // before the user submits.
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final ctrl = Get.find<TaskController>();
      final args = Get.arguments;
      if (args is Map) {
        final relType = args['rel_type']?.toString();
        final relId = args['rel_id']?.toString();
        if (relType != null && relType.isNotEmpty) {
          ctrl.taskRelatedController.text = relType;
        }
        if (relId != null && relId.isNotEmpty) {
          ctrl.relationIdController.text = relId;
        }
        ctrl.update();
      }
    });
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Padding(
        padding: EdgeInsets.fromLTRB(
            ...
            Dimensions.space8,
            Dimensions.space10,
            MediaQuery.viewPaddingOf(context).bottom + Dimensions.space8),
