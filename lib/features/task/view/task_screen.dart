...
import 'package:flutex_admin/common/components/no_permission.dart';
...
import 'package:flutex_admin/features/task/widget/task_kanban.dart';
...
class _TaskScreenState extends State<TaskScreen> {
  ...
  List<Task> _filteredTasks(TaskController controller) {
    final list = List<Task>.from(controller.tasksModel.data ?? <Task>[]);

    final filtered = list.where((task) {
      final statusOk = selectedStatus == null ||
          selectedStatus!.isEmpty ||
          Converter.taskStatusString(task.status ?? '').toLowerCase() ==
              selectedStatus!.toLowerCase();
      final priorityOk = selectedPriority == null ||
          selectedPriority!.isEmpty ||
          Converter.taskPriorityString(task.priority ?? '').toLowerCase() ==
              selectedPriority!.toLowerCase();
      return statusOk && priorityOk;
    }).toList();

    filtered.sort((a, b) {
      final aDate = DateTime.tryParse((a.dateAdded ?? '').trim()) ??
          DateTime.fromMillisecondsSinceEpoch(0);
      final bDate = DateTime.tryParse((b.dateAdded ?? '').trim()) ??
          DateTime.fromMillisecondsSinceEpoch(0);
  ...
  Future<void> _openFilterSheet(TaskController controller) async {
    final tasks = controller.tasksModel.data ?? <Task>[];
    final statusOptions = tasks
        .map((e) => Converter.taskStatusString(e.status ?? '').trim())
        .where((e) => e.isNotEmpty)
        .toSet()
        .toList()
      ..sort();
    final priorityOptions = tasks
        .map((e) => Converter.taskPriorityString(e.priority ?? '').trim())
        .where((e) => e.isNotEmpty)
        .toSet()
        .toList()
      ..sort();
  ...
  Widget build(BuildContext context) {
    return GetBuilder<TaskController>(builder: (controller) {
      ...
      final filteredList = _filteredTasks(controller);
      final isFilterActive = selectedStatus != null || selectedPriority != null;

      return Scaffold(
        backgroundColor:
            isDark ? const Color(0xFF000000) : const Color(0xFFDCE3EE),
        floatingActionButton: controller.hasPermissionError
            ? null
            : AnimatedSlide(
        ...
        body: controller.isLoading
            ? const CustomLoader()
            : controller.hasPermissionError
                ? NoPermissionWidget(onRetry: () => controller.initialData())
                : RefreshIndicator(
                                      ...
                                      child: ListView.separated(
                                        ...
                                        itemBuilder: (context, index) {
                                          final item = controller
                                              .tasksModel.overview![index];
                                          final label = item.status ?? '';
                                          final isSelected = selectedStatus !=
                                                  null &&
                                              selectedStatus!.toLowerCase() ==
                                                  label.toLowerCase();
                                          return _SummaryChip(
                                ...
                                if (_kanbanMode)
                                  SizedBox(
                                    height: MediaQuery.of(context).size.height *
                                        0.62,
                                    child: TaskKanban(tasks: filteredList),
                                  )
                                else
