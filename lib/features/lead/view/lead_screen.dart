...
import 'package:flutex_admin/features/lead/widget/lead_kanban.dart';
...
class _LeadScreenState extends State<LeadScreen> {
  ...
  List<Lead> _filteredLeads(LeadController controller) {
    final list = List<Lead>.from(controller.leadsModel.data ?? <Lead>[]);
    if (selectedStatus == null || selectedStatus!.isEmpty) return list;
    return list
        .where((l) =>
            (l.statusName ?? '').trim().toLowerCase() ==
            selectedStatus!.toLowerCase())
        .toList();
  }

  Future<void> _openFilterSheet(LeadController controller) async {
    final leads = controller.leadsModel.data ?? <Lead>[];
    final statusOptions = leads
  ...
  Future<void> _confirmBulkDelete(
      BuildContext ctx, LeadController controller) async {
    final ok = await showDialog<bool>(
      context: ctx,
      builder: (c) => AlertDialog(
        title: const Text('Delete Leads?'),
        content: Text(
            'Delete ${controller.selectedLeadIds.length} selected lead(s)? This cannot be undone.'),
        actions: [
          TextButton(
    ...
    if (ok == true) controller.bulkDeleteLeads();
  }

  Future<void> _showBulkStatusSheet(
      BuildContext ctx, LeadController controller) async {
    if (controller.leadStatusesAdminList.isEmpty) {
      await controller.loadLeadStatusesAdmin();
    }
    if (!mounted) return;
    await showModalBottomSheet<void>(
      context: ctx,
        ...
        child: ListView(
          ...
          children: [
            const Padding(
              ...
              child: Text('Change status for selected leads',
                  style: semiBoldDefault),
            ),
            const Divider(height: 1),
            ...controller.leadStatusesAdminList.map((s) => ListTile(
                  leading: const Icon(Icons.label_outline),
                  title: Text(s['name']?.toString() ?? ''),
                  onTap: () {
                    Navigator.pop(c);
                    controller.bulkUpdateStatus(s['id']?.toString() ?? '');
                  },
                )),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return GetBuilder<LeadController>(builder: (controller) {
      ...
      final filteredList = _filteredLeads(controller);
      final activeFilterCount = selectedStatus != null ? 1 : 0;

      return Scaffold(
        backgroundColor:
            isDark ? const Color(0xFF000000) : const Color(0xFFDCE3EE),
        bottomNavigationBar: controller.isSelectionMode
            ? _SelectionActionBar(
                count: controller.selectedLeadIds.length,
                onCancel: () => controller.clearSelection(),
                onDelete: () => _confirmBulkDelete(context, controller),
                onChangeStatus: () => _showBulkStatusSheet(context, controller),
              )
            : null,
                        ...
                        child: Column(
                          children: [
                            _GlassHeader(
                              ...
                              onManageSources: () =>
                                  Get.toNamed(RouteHelper.leadSourcesScreen),
                              onManageStatuses: () =>
                                  Get.toNamed(RouteHelper.leadStatusesScreen),
                              onImportCsv: () =>
                                  Get.toNamed(RouteHelper.importLeadsScreen),
                            ),
                            ...
                            if (controller.leadsModel.overview != null &&
                                controller.leadsModel.overview!.isNotEmpty)
                              Padding(
                            ...
                            if (_kanbanMode)
                              SizedBox(
                                ...
                                child: LeadKanban(
                                  leads: filteredList,
                                  statusColumns: controller
                                          .leadStatusesAdminList.isNotEmpty
                                      ? controller.leadStatusesAdminList
                                      : null,
                                  onStatusChange: (lead, newStatusId) =>
                                      controller.updateLeadStatusInline(
                                          lead.id ?? '', newStatusId),
                                ),
                              )
                              ...
                              ListView.separated(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return LeadCard(
                                      index: index,
                                      leadModel: LeadsModel(data: filteredList),
                                      isSelected: isSelected,
                                      onLongPress: () {
                                        if (controller.isSelectionMode) return;
                                        controller
                                            .enterSelectionMode(lead.id ?? '');
                                      },
                                      onTapOverride: controller.isSelectionMode
                                          ? () =>
                                              controller.toggleLeadSelection(
                                                  lead.id ?? '')
                                          : null,
                                    );
                                  },
...
class _GlassHeader extends StatelessWidget {
  ...
  final VoidCallback? onManageStatuses;
          ...
          child: Row(
            children: [
              ...
              if (onManageSources != null ||
                  onManageStatuses != null ||
                  onImportCsv != null)
                PopupMenuButton<String>(
                  icon: const Icon(Icons.more_vert_rounded),
                  onSelected: (value) {
                    if (value == 'sources') onManageSources?.call();
                    if (value == 'statuses') onManageStatuses?.call();
                    if (value == 'import') onImportCsv?.call();
                  },
                  itemBuilder: (ctx) => [
                    if (onManageSources != null)
                      const PopupMenuItem(
