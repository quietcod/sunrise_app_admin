...
class _ProjectTasksState extends State<ProjectTasks> {
  ...
  void _openAddTask() {
    // Ensure TaskController is registered, then preselect this project.
    Get.put(ApiClient(sharedPreferences: Get.find()));
    Get.put(TaskRepo(apiClient: Get.find()));
    final taskCtrl = Get.put(TaskController(taskRepo: Get.find()));
    taskCtrl.clearData();
    taskCtrl.taskRelatedController.text = 'project';
    taskCtrl.relationIdController.text = widget.id;
    Get.toNamed(
      RouteHelper.addTaskScreen,
      arguments: {'rel_type': 'project', 'rel_id': widget.id},
    )?.then((_) {
      // Refresh the list after returning.
      Get.find<ProjectController>().loadProjectGroup(widget.id, 'tasks');
    });
  }
