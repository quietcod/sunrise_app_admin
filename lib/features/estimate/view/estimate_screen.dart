...
import 'package:flutex_admin/features/estimate/widget/estimate_kanban.dart';
...
class _EstimateScreenState extends State<EstimateScreen> {
  ...
  List<Estimate> _filteredEstimates(EstimateController controller) {
    final list =
        List<Estimate>.from(controller.estimatesModel.data ?? <Estimate>[]);
    if (selectedStatus == null || selectedStatus!.isEmpty) return list;
    return list
        .where((e) =>
            (e.statusName ?? '').trim().toLowerCase() ==
            selectedStatus!.toLowerCase())
        .toList();
  }

  Future<void> _openFilterSheet(EstimateController controller) async {
    final estimates = controller.estimatesModel.data ?? <Estimate>[];
    final statusOptions = estimates
  ...
  Widget build(BuildContext context) {
    return GetBuilder<EstimateController>(builder: (controller) {
      ...
      final filteredList = _filteredEstimates(controller);
        ...
        floatingActionButton: AnimatedSlide(
          ...
          child: AnimatedOpacity(
            ...
            child: CustomFAB(
              isShowIcon: true,
              isShowText: false,
              press: () => Get.toNamed(RouteHelper.addEstimateScreen),
            ),
          ),
        ),
                        ...
                        child: Column(
                          children: [
                            ...
                            if (controller.estimatesModel.overview != null &&
                                controller.estimatesModel.overview!.isNotEmpty)
                              Padding(
                            ...
                            if (_kanbanMode)
                              SizedBox(
                                ...
                                child: filteredList.isNotEmpty
                                    ? EstimateKanban(estimates: filteredList)
                                    : const NoDataWidget(),
                              )
                            else
