              ...
              children: [
                ...
                Builder(builder: (context) {
                  ...
                  void add(Widget tile) {
                    if (tiles.isNotEmpty) {
                      tiles.add(const CustomDivider(space: Dimensions.space10));
                    }
                    tiles.add(tile);
                  }
                  ...
                  if (MyPermissions.canManageSettings) {
                    add(MenuItems(
                        ...
                        label: LocalStrings.adminSettings.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.settingsHubScreen)));
                  }
                  if (MyPermissions.canViewKb) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.knowledgeBase.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.kbGroupsScreen)));
                  }
                  if (MyPermissions.canViewSubscriptions) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.subscriptions.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.subscriptionsScreen)));
                  }
                  if (MyPermissions.canViewReports) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.reports.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.reportsScreen)));
                  }
                  if (MyPermissions.canViewCalendar) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.calendar.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.calendarScreen)));
                  }
                  if (MyPermissions.canViewNewsfeed) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.newsfeed.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.newsfeedScreen)));
                  }
                  if (MyPermissions.canViewGdpr) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.gdpr.tr,
                        onPressed: () => Get.toNamed(RouteHelper.gdprScreen)));
                  }
                  if (MyPermissions.canViewEstimateRequests) {
                    add(MenuItems(
                        imageSrc: MyImages.policy,
                        label: LocalStrings.estimateRequests.tr,
                        onPressed: () =>
                            Get.toNamed(RouteHelper.estimateRequestsScreen)));
                  }
                ...
                const SizedBox(height: Dimensions.space10),
                // Staff Tools section (non-admin only)
                        ...
                        children: [
                          ...
                          MenuItems(
                              ...
                              onPressed: () => Get.toNamed(
                                  RouteHelper.attendanceRecordsScreen)),
                        ],
                      )),
                  const SizedBox(height: Dimensions.space10),
                ],
