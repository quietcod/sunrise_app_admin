...
class ExpenseCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Billable expenses get orange, non-billable get slate
    final isBillable = (expense.billable ?? '0') == '1';
    final statusColor =
        isBillable ? ColorResources.colorOrange : ColorResources.darkColor;

    final p = _palette(statusColor);
    final name = expense.expenseName ?? '';
    final category = expense.categoryName ?? '';
    final amount = '${expense.currencySymbol ?? ''}${expense.amount ?? '0'}';
    final date = expense.date ?? '';
    final client = expense.clientName ?? '';
    final project = expense.projectName ?? '';
    final billableLabel = isBillable ? 'Billable' : 'Not Billable';
          ...
          child: Container(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  // Row 1: icon + name + amount + billable chip
                  Row(
                    children: [
                      Container(
                        width: 40,
