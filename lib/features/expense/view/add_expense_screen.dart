...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  void initState() {
    super.initState();
    // Ensure ExpenseController is registered (route may be opened directly
    // from project expenses tab without prior binding).
    if (!Get.isRegistered<ApiClient>()) {
      Get.put(ApiClient(sharedPreferences: Get.find()));
    }
    if (!Get.isRegistered<ExpenseRepo>()) {
      Get.put(ExpenseRepo(apiClient: Get.find()));
    }
    if (!Get.isRegistered<ExpenseController>()) {
      Get.put(ExpenseController(expenseRepo: Get.find()));
    }
    // If opened from a project's expenses tab, pre-select that project.
    final args = Get.arguments;
    if (args is Map && args['project_id'] != null) {
      _preselectedProjectId = args['project_id'].toString();
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Get.find<ExpenseController>().projectController.text =
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(
        title: LocalStrings.addExpense.tr,
      ),
              ...
              child: Column(
                ...
                children: [
                    ...
                    builder: (context, snap) {
                      if (snap.data?.status ?? false) {
                        ...
                        return CustomDropDownTextField(
                          ...
                          dropDownColor: Theme.of(context).cardColor,
                          selectedValue: _preselectedProjectId,
                          onChanged: (value) {
                  ...
                  controller.isSubmitLoading
                      ...
                      : RoundedButton(
                          text: LocalStrings.addExpense.tr,
