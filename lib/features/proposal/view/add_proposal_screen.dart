...
class _AddProposalScreenState extends State<AddProposalScreen> {
  ...
  void initState() {
    ...
    _projectId = (args is Map && args['projectId'] != null)
    ...
    if (!Get.isRegistered<ProposalController>()) {
      Get.put(ProposalController(proposalRepo: Get.find()));
    }
    final c = Get.find<ProposalController>();
    c.isLoading = false;
    c.isSubmitLoading = false;
    WidgetsBinding.instance.addPostFrameCallback((_) => c.update());
  }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (controller.proposalRelatedController.text == 'lead')
                    FutureBuilder(
                        ...
                        builder: (context, leadList) {
                          ...
                          } else if (leadList.connectionState ==
                  ...
                  // Contacts: loader / dropdown (multiple) / nothing (single auto-filled)
                  if (controller.proposalRelatedController.text == 'customer' &&
                      controller.selectedCustomerId.isNotEmpty)
                    controller.isContactsLoading
                        ? const Padding(
                            padding: EdgeInsets.symmetric(
                                vertical: Dimensions.space8),
                            child: CustomLoader(isFullScreen: false),
                          )
                        : (controller.contactsModel.data != null &&
                                controller.contactsModel.data!.length > 1)
                            ? CustomDropDownTextField(
                                hintText: LocalStrings.selectContact.tr,
                                onChanged: (value) {
                                  controller
                                      .onContactSelected(value as Contact);
                                },
            ...
            : controller.isSubmitLoading
                ...
                : Padding(
                    ...
                    child: RoundedButton(
                      ...
                      press: () {
                        controller.submitProposal(projectId: _projectId);
                      },
                    ),
                  );
      }),
